...
export const CartButton = () => {
    ...
    return (
        <>
            {store.isAuthenticated && (
                <button className={`btn btn-primary ${isOffcanvasActive ? "active" : ""}`} type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight" onClick={toggleOffcanvas}>
                    {totalQuantity} <TiShoppingCart style={{ fontSize: "2rem" }}/>
                </button>
            )}

            <div className={`offcanvas offcanvas-end ${isOffcanvasActive ? "show" : ""}`} tabIndex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel" ref={offcanvasRef}>
                ...
                <div className="offcanvas-body">
                    <ul className="list-group">
                        {store.cart.map((item) => (
                            <li key={item.id} className="list-group-item d-flex justify-content-between align-items-center cart-item" style={{ backgroundColor: '#ffd5cd', border: '1px solid #ff7e67', margin: '5px 0' }}>
                                <span>{item.name} <br/>Cantidad: {item.quantity} <br/> Precio: {(item.price * item.quantity).toFixed(2)} €</span>
